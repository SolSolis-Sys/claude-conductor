...
const { resolveDependencies } = require('../../conductor-blueprints/lib/dependency-resolver');
...
async function discover() {
  ...
  try {
    ...
    // Build category index from tags
    const categoryMap = {
      'code-quality': ['adversarial-review', 'pre-push-cohesion-check'],
      'testing': ['tdd-bug-hunter'],
      'ci-cd': ['ci-polling', 'deploy-verify'],
      'planning': ['idea-to-spec', 'brainstorming-premortem'],
      'self-improvement': ['self-skill-improvement', 'conductor-loop-improvement']
    };
    ...
    if (category === '1') {
      ...
      recommendedNames = categoryMap['code-quality'] || [];
    } else if (category === '2') {
      selectedCategory = 'testing';
      recommendedNames = categoryMap['testing'] || [];
    } else if (category === '3') {
      selectedCategory = 'planning';
      recommendedNames = categoryMap['planning'] || [];
    } else if (category === '4') {
      selectedCategory = 'ci-cd';
      recommendedNames = categoryMap['ci-cd'] || [];
    } else if (category === '5') {
      selectedCategory = 'self-improvement';
      recommendedNames = categoryMap['self-improvement'] || [];
    } else if (category === '6') {
      // Show all blueprints
      console.log('\nAll blueprints:');
...
module.exports = { list, search, install, info, submit, discover, submitInteractive };
