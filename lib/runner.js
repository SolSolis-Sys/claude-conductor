...
function dryRun(blueprint, inputs, options) {
  ...
  // Resolve v1→v1.1 coercion so agents[] blueprints work in dry-run too
  const resolved   = _resolveBlueprint(blueprint);
  const gates      = resolved.gates.length > 0 ? resolved.gates
                     : (Array.isArray(blueprint.gates) ? blueprint.gates : []);
  const allWarnings = [];
  const output = [];
  const logLines = [];
...
// ── run() — Execute blueprint: validate + build plan ──────────────────────

/**
 * Resolve {{variables}} in a template string using a simple inputs map.
 * Variables not found in inputs are left as-is ({{variable_name}}).
 * Returns the resolved string directly (unlike resolveVariables which returns {resolved, warnings}).
 *
 * @param {string} template
 * @param {object} inputs
 * @returns {string}
...
function resolveInputs(template, inputs) {
  ...
  return template.replace(/\{\{(\w+)\}\}/g, (match, key) =>
    Object.prototype.hasOwnProperty.call(safeInputs, key) ? String(safeInputs[key]) : match
  );
}

/**
 * Build and return an execution plan for a blueprint without actually running it.
 *
 * Supports both v1.0 (agents[]) and v1.1 (gates[]) blueprints:
 *   - v1.1 (gates[] only): coerces gates[] → agents[] for plan building
 *   - v1.0 (agents[] only): validates as-is (gates[] optional in schema), plans from agents[]
 *
 * Returns:
 *   { ok: true, name, version, plan }   — success
 *   { ok: false, errors }               — validation failure
 *
 * @param {object} blueprint   — blueprint object (parsed, not file path)
 * @param {object} [inputs={}] — variable substitution map ({{key}} → value)
 * @returns {{ ok: boolean, name?: string, version?: string, plan?: object[], errors?: object[] }}
 */
function run(blueprint, inputs) {
  const safeInputs = (inputs && typeof inputs === 'object') ? inputs : {};

  // Work on a shallow copy to avoid mutating caller's object
  const bp = Object.assign({}, blueprint);

  // Coerce v1.1 gates[] → agents[] so plan building always uses bp.agents
  if (!Array.isArray(bp.agents) && Array.isArray(bp.gates)) {
    bp.agents = bp.gates;
  }

  // Validate — validateGates requires blueprint.name and, if gates[] present, validates each gate
  const validation = validateGates(bp);
  if (!validation.valid) {
    return { ok: false, errors: validation.errors };
  }

  const agents = Array.isArray(bp.agents) ? bp.agents : [];

  const plan = agents.map((gate, i) => ({
    index:   i + 1,
    id:      gate.id   || gate.role || `step-${i + 1}`,
    type:    gate.type || 'agent',
    prompt:  gate.prompt  ? resolveInputs(gate.prompt, safeInputs)  : null,
    command: gate.command ? resolveInputs(gate.command, safeInputs) : null,
    model:   gate.model || 'sonnet',
  }));

  return { ok: true, name: bp.name, version: bp.version, plan };
}

module.exports = { loadBlueprint, validateGates, resolveVariables, resolveInputs, dryRun, validateInterGate, validateLoop, run };
