...
/**
 * Normalize a description string for single-line display.
 * Replaces CR/LF sequences with a space and collapses multiple spaces.
 * @param {string} s
 * @returns {string}
 */
...
/**
 * Fetch a URL with timeout, returns raw text string or throws.
 * @param {string} url
...
async function list() {
  ...
  const descW = Math.max(11, ...blueprints.map((b) => normalizeDesc(b.description).length));
  ...
  for (const b of blueprints) {
    console.log(row(b.name || '', normalizeDesc(b.description), (b.tags || []).join(', '), b.cost_tier || ''));
  }
  console.log(sep);
...
async function search(query) {
  ...
  const descW = Math.max(11, ...matches.map((b) => normalizeDesc(b.description).length));
  ...
  for (const b of matches) {
    ...
    console.log(`${(b.name || '').padEnd(nameW)}  ${normalizeDesc(b.description).padEnd(descW)}  ${tags}${cost}`);
  }
  console.log(`\nInstall: conductor hub install <name>\n`);
}
...
async function install(name, options = {}) {
  ...
  // Try to fetch README.md — optional, 404 is acceptable
  const readmeUrl = `${BASE_URL}/blueprints/${name}/README.md`;
  try {
    const readmeContent = await fetchText(readmeUrl);
    fs.writeFileSync(path.join(destDir, 'README.md'), readmeContent, 'utf8');
  } catch (e) {
    if (!e.message.startsWith('Not found')) {
      // Non-404 error — warn but do not block the install
      console.warn(`[hub] warning: could not fetch README.md for '${name}': ${e.message}`);
    }
    // 404 = README absent in repo, silently skip
  }
